...
class IntermediateLayerGetter(nn.ModuleDict):
    ...
    def __init__(self, model, return_layers):
        ...
        # traversal the model submodules and store them in an ordered dictionary
...
def resnet50_fpn_backbone(pretrain_path="",
    ...
    Args:
        pretrain_path: pretraining weights for resnet50, default to null if not used
        norm_layer: the official default is FrozenBatchNorm2d,the parameter of the BN layer is not updated
        trainable_layers: specifies which layers to train
        returned_layers: specify which output to return
        extra_blocks: add additional layer structure
    ...
    # record each feature layer channel that resnet50 provides to FPN
    in_channels_list = [in_channels_stage2 * 2 ** (i - 1) for i in returned_layers]
    # Channel of each feature layer
    out_channels = 256
